...
export class EditableComponent implements OnInit {
  ...
  editing = false;
  open: boolean[] = [];
  tiles: Subject<ContentItem[] | null> = new BehaviorSubject(null);
  ...
  percentDetectedByMedicalVisit: number;
  percentInfectedMedicalVisitsTested: number;
  percentInfectedSeekingMedicalTreatment: number;
  daysInfectionToOnset: number;
  daysOnsetToPositive: number;
  percentDailyGrowth: number;
  daysOnsetToDeath: number;
  percentInfectedFatality: number;
  ...
  initializeForm(event?: MatSelectChange) {
    ...
    this.formSubscription = this.form.valueChanges.pipe(debounce(() => interval(1000))).subscribe(value => {
      Object.assign(this, value);
      this.setTiles();
    })
    Object.assign(this, this.form.value);
    this.setTiles();
  }
  ...
  setTiles() {
    this.tiles.next([{
      ...
      formItem: {
        ...
        control: 'date',
        type: 'select',
        options: this.possibleDates
      }
    }, {
      header: `Confirmed cases reported on ${this.date}`,
      value: this.reportedConfirmedCases,
      formItem: {
        label: 'Reported Confirmed Cases',
        control: 'reportedConfirmedCases',
        type: 'number',
        step: 1
      }
    }, {
      ...
      formItem: {
        label: 'Percent Detected by Medical Visit',
        control: 'percentDetectedByMedicalVisit',
        type: 'number',
        step: .01
      }
    }, {
      ...
      formItem: {
        label: 'Percent Infected Medical Visits Tested',
        control: 'percentInfectedMedicalVisitsTested',
        type: 'number',
        step: .01
      }
    }, {
      ...
      formItem: {
        label: 'Percent Infected Seeking Medical Treatment',
        control: 'percentInfectedSeekingMedicalTreatment',
        type: 'number',
        step: .01
      }
    }, {
      ...
      formItem: {
        label: 'Days Infection to Onset',
        control: 'daysInfectionToOnset',
        type: 'number',
        step: 1
      }
    }, {
      ...
      formItem: {
        label: 'Days Onset to Positive',
        control: 'daysOnsetToPositive',
        type: 'number',
        step: 1
      }
    }, {
      ...
      formItem: {
        label: 'Percent Daily Growth',
        control: 'percentDailyGrowth',
        type: 'number',
        step: .01
      }
    }, {
      ...
      formItem: {
        label: 'Days Onset to Death',
        control: 'daysOnsetToDeath',
        type: 'number',
        step: 1
      }
    }, {
      ...
      formItem: {
        label: 'Percent Infected Fatality',
        control: 'percentInfectedFatality',
        type: 'number',
        step: .0001
      }
    }, {
      header: `Estimated minimum number ${this.region} deaths based on ${this.numberPipe.transform(Math.floor(this.estimatedInfectedOnDate), '.0')} infected`,
